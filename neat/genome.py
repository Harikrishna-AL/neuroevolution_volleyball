...
class Genome:
    def __init__(
        ...
        self.nodes = (
            jnp.empty((0, 2), dtype=jnp.int32) if nodes is None else jnp.array(nodes)
        )
        self.connections = (
            jnp.empty((0, 5), dtype=jnp.float32)
            if connections is None
            else jnp.array(connections)
        )
        ...
        def _init_pop(key):
            ...
            conn = conn.at[:, 0].set(
                jnp.tile(jnp.arange(input_num, dtype=jnp.int32), output_num)
            )
            conn = conn.at[:, 1].set(
                jnp.repeat(
                    jnp.arange(input_num, input_num + output_num, dtype=jnp.int32),
                    input_num,
                )
            )
            ...
            conn = conn.at[:, 3].set(
                jnp.arange(input_num * output_num, dtype=jnp.int32)
            )
            conn = conn.at[:, 4].set(
                jax.random.bernoulli(key, p=0.5, shape=(input_num * output_num,))
            )

            conn = conn.at[:, 2].set(
                jax.random.uniform(
                    key, shape=(input_num * output_num,), minval=-1.0, maxval=1.0
                )
            )
            ...
            matrix = matrix.at[source_nodes, target_nodes].add(
                enabled_connections * weights
            )

            return GenomeData(
                nodes, conn, input_num * output_num, input_num + output_num, key, matrix
            )
        ...
        def _forward(matrix, obs):
            ...
            for i in range(n_inputs, num_nodes_static):
                ...
                activation_idx = jax.random.randint(
        ...
        def __add_node(genome: GenomeData, key):
            ...
            new_matrix = new_matrix.at[: genome.node_count, : genome.node_count].set(
                genome.matrix
            )
            # genome.node_count += 1
            return new_id, GenomeData(
                new_nodes,
                genome.connections,
        ...
        def __add_connection(genome: GenomeData, in_node, out_node, weight, key):
            new_connection = jnp.array(
                [[in_node, out_node, weight, genome.innovation_count, 1.0]]
            )
            new_connections = jnp.concatenate(
                (genome.connections, new_connection), axis=0
            )
            # genome.innovation_count += 1
            matrix = genome.matrix.at[int(in_node), int(out_node)].set(weight)

            return genome.innovation_count + 1, GenomeData(
                genome.nodes,
                new_connections,
                genome.innovation_count + 1,
                genome.node_count,
        ...
        def __mutate_add_node(genome: GenomeData):
            ...
            inv_num, genome = self.add_connection(
                genome, connection_to_split[0], new_node_id, 1.0, subkey
            )
            inv_num, genome = self.add_connection(
                genome,
                new_node_id,
                connection_to_split[1],
                connection_to_split[2],
                subkey,
            )
        ...
        def __mutate_add_connection(genome: GenomeData, weight_range=(-1.0, 1.0)):
            ...
            matrix = genome.matrix.at[int(node_input[0]), int(node_output[0])].set(
                weight
            )
            return (
                GenomeData(
        ...
        def pad_matrix(matrix, max_nodes):
            ...
            max_nodes = int(
                jax.lax.stop_gradient(max_nodes)
            )  # Convert to a concrete integer

            diff = max_nodes - matrix.shape[0]
            if diff > 0:
                new_matrix = jnp.zeros((max_nodes, max_nodes))
                new_matrix = new_matrix.at[: matrix.shape[0], : matrix.shape[1]].set(
                    matrix
                )
                return new_matrix
    ...
    def mutate(self, genome: GenomeData):
        ...
        reenable = jax.random.bernoulli(
            subkey, p=self.config["prob_enable"], shape=(disabled.shape[0],)
        )
        connections = connections.at[disabled, 4].set(reenable)

        mutated_weights = jax.random.bernoulli(
            subkey, p=self.config["prob_weight_mut"], shape=(n_conn,)
        )
        weights_change = (
            jax.random.uniform(subkey, shape=(n_conn,), minval=-1.0, maxval=1.0)
            * mutated_weights
        )
        connections = connections.at[:, 2].add(weights_change)

        connections = connections.at[:, 2].set(
            jnp.clip(
                connections[:, 2],
                -self.config["clamp_weights"],
                self.config["clamp_weights"],
            )
        )
        ...
        if jax.random.uniform(subkey, shape=(), minval=0.0, maxval=1.0) < self.config[
            "prob_add_node"
        ] and jnp.any(connections[:, 4] == 1.0):
            genome, innov = self.mutate_add_node(
                GenomeData(
                    genome.nodes,
                    genome.connections,
                    genome.innovation_count,
        ...
        if (
            jax.random.uniform(subkey, shape=(), minval=0.0, maxval=1.0)
            < self.config["prob_add_connection"]
        ):
            genome, innov = self.mutate_add_connection(
                GenomeData(
                    genome.nodes,
                    genome.connections,
                    genome.innovation_count,
    ...
    def topological_sort(self, nodes):
        ...
        sorted_nodes = jnp.concatenate(
            (input_nodes, hidden_nodes, output_nodes), axis=0
        )
        return sorted_nodes
    ...
    def visualize(self, genome: GenomeData):
        ...
        for connection in connections:
            if connection[4] == 1.0:
                G.add_edge(
                    int(connection[0]), int(connection[1]), weight=float(connection[2])
                )

        pos = nx.spring_layout(G)
        node_colors = [
            "green" if node[1] == 0 else "blue" if node[1] == 1 else "orange"
            for node in nodes
        ]
        ...
        edge_labels = {
            (int(connection[0]), int(connection[1])): round(float(connection[2]), 2)
            for connection in connections
        }
